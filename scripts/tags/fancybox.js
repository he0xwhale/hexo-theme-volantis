...
function buidImgFancybox(content, group) {
    ...
    imgList.forEach(item => {
        ...
        const result = `<div class='fancybox'><a class='fancybox' itemscope itemtype="https://schema.org/ImageObject" itemprop="url" href='${url}' data-fancybox='${group}' data-caption='${alt}'>${newItem}</a>${buidAlt(imageTags || alt)}</div>`;
...
function postFancybox(args, content) {
    ...
    if (col > 0) {
        return `<div galleryFlag itemscope itemtype="https://schema.org/ImageGallery" class="gallery ${cls}" col='${col}' data-group='${group}'>${buidImgFancybox(content, group)}</div>`;
    }
    return `<div galleryFlag itemscope itemtype="https://schema.org/ImageGallery" class="gallery ${cls}" data-group='${group}'>${buidImgFancybox(content, group)}</div>`;
}
